...
export default function CheckoutSuccessPage() {
  ...
  return <CheckoutShell><div className="checkout-status-card text-center"><div className="checkout-success-icon"><CheckCircle2 /></div><h1>Subscription Successful!</h1><p className="checkout-status-copy">{storeExists ? 'Your payment has been processed successfully. Your store is ready to use.' : 'Your payment has been processed successfully. Create your store to finish setup.'}</p><div className="checkout-details text-left"><Detail label="Selected plan" value={plan} strong /><Detail label="Invoice number" value={invoice} /><Detail label="Renewal date" value={renewal} /></div><Link to={destination} className="checkout-primary-action">{storeExists ? 'Go to Dashboard' : 'Create My Store'} <ArrowRight className="h-5 w-5" /></Link>{invoiceUrl && <a href={invoiceUrl} target="_blank" rel="noreferrer" className="checkout-secondary-action"><Download className="h-4 w-4" />Download invoice</a>}</div></CheckoutShell>;
}
