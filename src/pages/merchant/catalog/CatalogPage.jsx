...
export default function CatalogPage() {
  ...
  return <div className="p-6 bg-surface min-h-screen text-on-surface">
    ...
    <div className="bg-white border border-outline-variant/40 rounded-xl overflow-hidden shadow-sm"><div className="px-6 border-b border-outline-variant/40"><nav className="flex space-x-8"><button onClick={() => setActiveTab('inventory')} className={`py-4 px-1 text-xs font-bold transition-all border-b-2 ${activeTab === 'inventory' ? 'text-primary border-primary' : 'text-on-surface-variant border-transparent hover:text-primary'}`}>All Inventory</button><button onClick={() => setActiveTab('bundles')} className={`py-4 px-1 text-xs font-medium transition-all border-b-2 ${activeTab === 'bundles' ? 'text-primary border-primary' : 'text-on-surface-variant border-transparent hover:text-primary'}`}>Bundles</button><button onClick={() => setActiveTab('recommendations')} className={`py-4 px-1 text-xs font-medium transition-all border-b-2 ${activeTab === 'recommendations' ? 'text-primary border-primary' : 'text-on-surface-variant border-transparent hover:text-primary'}`}>AI Recommendations</button></nav></div>{activeTab === 'inventory' ? <ProductsTable storeId={storeId} categories={categories} activeCategory={activeCategory} onSummaryChange={setSummary} /> : activeTab === 'bundles' ? <BundlesTab storeId={storeId} /> : <ProductRecommendationsTab storeId={storeId} />}</div>
  </div>;
}
