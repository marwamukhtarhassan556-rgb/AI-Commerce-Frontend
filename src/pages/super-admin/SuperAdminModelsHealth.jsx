...
const providerMeta = {
  ...
  openai: { label: 'OpenAI', icon: 'smart_toy', color: '#10B981' },
  azure: { label: 'Azure AI', icon: 'cloud', color: '#38BDF8' },
  mistral: { label: 'Mistral', icon: 'air', color: '#A78BFA' },
  deepseek: { label: 'DeepSeek', icon: 'travel_explore', color: '#0EA5E9' },
  ollama: { label: 'Ollama', icon: 'dns', color: '#94A3B8' },
};
  ...
  return (
    <div className="p-8 space-y-8">
      <div className="flex items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold" style={{ color: 'var(--color-on-surface)' }}>AI Models & Provider Health</h2>
          <p className="mt-1 text-sm" style={{ color: 'var(--color-on-surface-variant)' }}>Provider liveness, latency, and model capability coverage from the AI service.</p>
        </div>
      ...
      {overall && (
        <section className="admin-card admin-border rounded-xl p-5 shadow-sm flex items-center gap-4">
          <StatusDot status={overall.status} />
          <div>
            <p className="font-semibold" style={{ color: 'var(--color-on-surface)' }}>Default AI Provider</p>
      ...
      <section className="grid gap-4" style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))' }}>
        {providers.map((provider) => {
          ...
          return (
            <article key={provider} className="admin-card admin-border rounded-xl p-5 shadow-sm">
              <div className="mb-3 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <span className="material-symbols-outlined" style={{ color: meta.color }}>{meta.icon}</span>
                  <span className="font-semibold" style={{ color: 'var(--color-on-surface)' }}>{meta.label}</span>
                </div>
                {health ? <StatusDot status={health.status} /> : <span className="text-xs" style={{ color: 'var(--color-on-surface-variant)' }}>Loading</span>}
              </div>
              <div className="space-y-1 text-sm" style={{ color: 'var(--color-on-surface-variant)' }}>
                <p>Status: <span className="font-semibold" style={{ color: 'var(--color-on-surface)' }}>{health?.status || 'checking'}</span></p>
                {health?.latency_ms !== undefined && <p>Latency: <span className="font-semibold" style={{ color: 'var(--color-on-surface)' }}>{health.latency_ms}ms</span></p>}
      ...
      <section className="admin-card admin-border rounded-xl shadow-sm overflow-hidden">
        <div className="border-b px-6 py-4" style={{ borderColor: 'var(--admin-border)' }}>
          <h3 className="font-semibold" style={{ color: 'var(--color-on-surface)' }}>Models Capability Matrix</h3>
        </div>
        {loading ? (
          <div className="p-8 text-center" style={{ color: 'var(--color-on-surface-variant)' }}>Loading models...</div>
        ) : models.length === 0 ? (
          <div className="p-8 text-center" style={{ color: 'var(--color-on-surface-variant)' }}>No model registry data available.</div>
        ) : (
            ...
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y" style={{ borderColor: 'var(--admin-border)' }}>
                {models.map((model, index) => {
                  ...
                  return (
                    <tr key={model.name || model.model_id || model.id || index} className="group transition-colors" style={{ background: 'var(--admin-card)' }} onMouseEnter={e => e.currentTarget.style.background='var(--color-surface-container-low)'} onMouseLeave={e => e.currentTarget.style.background='var(--admin-card)'}>
                      <td className="px-4 py-3 font-semibold" style={{ color: 'var(--color-on-surface)' }}>{model.name || model.model_id || model.id || '-'}</td>
                      <td className="px-4 py-3" style={{ color: 'var(--color-on-surface-variant)' }}>{model.provider || '-'}</td>
