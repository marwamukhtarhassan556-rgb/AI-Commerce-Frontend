  ...
  return (
    <div className="p-8 space-y-8">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <h2 className="text-2xl font-bold text-[#0D1B2A]">Bundle Promo Tracking</h2>
          <p className="mt-1 text-sm text-[#64748B]">Promo-copy events, top bundles, discount impact, and tracking threshold.</p>
        </div>
        <div className="flex flex-wrap gap-3">
          <button type="button" onClick={() => setTopOnly((value) => !value)} className="rounded-lg border border-[#CBD5E1] bg-white px-4 py-2 text-sm font-bold text-[#0D1B2A]">
      ...
      <section className="rounded-xl border border-[#E2E8F0] bg-white p-5 shadow-sm">
        <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div>
            <h3 className="font-semibold text-[#0D1B2A]">Tracking Config</h3>
            <p className="mt-1 text-sm text-[#64748B]">Current threshold: {config.threshold ?? 5}, tracking {config.enabled ? 'enabled' : 'disabled'}.</p>
          </div>
          <div className="flex flex-wrap items-end gap-3">
            <label className="text-xs font-bold uppercase tracking-wide text-[#64748B]">
              ...
              <input
                ...
                className="mt-1 block h-10 w-28 rounded-lg border border-[#CBD5E1] bg-white px-3 text-sm text-[#0D1B2A]"
              />
            </label>
            <label className="flex h-10 items-center gap-2 rounded-lg border border-[#CBD5E1] bg-white px-3 text-sm font-semibold text-[#0D1B2A]">
              <input
                type="checkbox"
      ...
      <section className="overflow-hidden rounded-xl border border-[#E2E8F0] bg-white shadow-sm">
        <div className="border-b border-[#E2E8F0] px-6 py-4">
          <h3 className="font-semibold text-[#0D1B2A]">Bundle Tracking Records</h3>
        </div>
        {loading ? (
          <div className="p-8 text-center text-[#64748B]">Loading bundles...</div>
        ) : bundles.length === 0 ? (
          <div className="p-8 text-center text-[#64748B]">No bundle copy events have been tracked yet.</div>
        ) : (
                ...
                {bundles.map((bundle, index) => {
                  ...
                  return (
                    <tr key={key} className="hover:bg-[#F8FAFC]">
                      <td className="px-4 py-3 font-semibold text-[#0D1B2A]">{key}</td>
                      <td className="px-4 py-3 text-[#64748B]">{bundle.promo_code || bundle.promoCode || '-'}</td>
                      <td className="px-4 py-3 text-[#64748B]">{Array.isArray(products) && products.length ? products.join(', ') : '-'}</td>
                      <td className="px-4 py-3 text-[#64748B]">
                        {bundle.discount_pct || bundle.discountPct || 0}% / {formatMoney(bundle.total_discount || bundle.totalDiscount)}
                      </td>
                      <td className="px-4 py-3 font-semibold text-[#0D1B2A]">{Number(bundle.copy_count || bundle.copyCount || 0).toLocaleString()}</td>
                      <td className="px-4 py-3 text-[#64748B]">{bundle.last_copied_at || bundle.lastCopiedAt || '-'}</td>
