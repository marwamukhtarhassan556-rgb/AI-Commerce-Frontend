...
const apiBaseUrl = 'https://aicommerce-ai-service-production.up.railway.app';
...
export default function WidgetAccessPanel({ framework = 'vanilla' }) {
  ...
  const code = useMemo(() => newKey ? `<script\n  src="${setup.src}"\n  data-widget-key="${newKey}"\n  data-api-base-url="${apiBaseUrl}"\n></script>` : '', [newKey, setup.src]);
  ...
  return <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
    ...
    <div className="widget-install-target"><b>2. Add the installation code</b><span>Paste it into <code>{setup.target}</code> after you create the key below.</span></div>
    <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><h2 className="flex items-center gap-2 font-bold text-slate-900"><KeyRound className="h-5 w-5 text-indigo-600" />Widget access</h2><p className="mt-1 text-sm text-slate-500">Create and manage the secure key for your {setup.name} storefront.</p></div><button type="button" disabled={creating || !origin} onClick={create} className="inline-flex min-h-10 items-center justify-center gap-2 rounded-lg bg-indigo-600 px-4 py-2 text-sm font-semibold text-white disabled:opacity-60"><Plus className="h-4 w-4" />{creating ? 'Creating…' : 'Create widget key'}</button></div>
    ...
    {newKey && <div className="mt-4 rounded-xl border border-emerald-200 bg-emerald-50 p-4"><p className="font-semibold text-emerald-900">Copy this key and installation code now. The key is shown only once.</p><CopyBox label="Widget key" value={newKey} copied={copied === 'key'} onCopy={() => copy(newKey, 'key')} /><CopyBox label={`Installation code for ${setup.name}`} value={code} copied={copied === 'code'} onCopy={() => copy(code, 'code')} multiline /></div>}
