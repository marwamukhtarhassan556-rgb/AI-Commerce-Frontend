...
export default function OverviewPage() {
  ...
  return <div className="p-8 max-w-[1100px] space-y-6"><div><h2 className="text-[28px] font-bold text-on-surface">Dashboard Overview</h2><p className="text-sm text-on-surface-variant mt-1">Real-time performance metrics for your active store.</p></div>{error && <p className="rounded-lg bg-red-50 px-4 py-3 text-sm text-error">{error}</p>}
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4"><div className="p-6 bg-white border border-outline-variant rounded-2xl shadow-sm"><div className="flex justify-between items-start mb-4"><div className="flex items-center gap-3"><div className="p-2.5 bg-primary-fixed rounded-xl"><CreditCard className="w-5 h-5 text-primary" /></div><div><p className="text-[10px] font-bold text-on-surface-variant uppercase tracking-widest">Total Revenue</p><p className="text-[11px] text-on-surface-variant">Live from dashboard API</p></div></div><div className={`flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold ${growth >= 0 ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-error'}`}><ArrowUp className={`w-3 h-3 ${growth < 0 ? 'rotate-180' : ''}`} /><span>{growth}%</span></div></div><div className="flex items-baseline gap-2"><span className="text-[40px] font-extrabold text-on-surface">{totalRevenue.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span><span className="text-base font-semibold text-on-surface-variant">{data.currency}</span></div></div>
