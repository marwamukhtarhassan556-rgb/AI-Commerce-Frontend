...
function StepHeading({ icon: Icon, eyebrow, title, copy, action }) { return <div className="integration-single-step"><div className="flex flex-col gap-4 border-b border-slate-100 pb-6 sm:flex-row sm:items-start sm:justify-between"><div><span className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-blue-600"><Icon className="h-4 w-4" />{eyebrow}</span><h1 className="mt-3 text-3xl font-extrabold text-slate-900">{title}</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">{copy}</p></div>{action}</div></div>; }
function SchemaGuideStep({ onBack, onContinue, onAskDeveloper }) { return <div><Back onClick={onBack} /><StepHeading icon={Sparkles} eyebrow="Integration · 1 of 4" title="Generate your OpenAPI schema" copy="Your schema tells Navi where your store API lives, how it logs in, and where to find products, orders, customers, categories, and coupons." action={<button type="button" onClick={onAskDeveloper} className="rounded-lg border border-blue-600 px-4 py-2 text-sm font-semibold text-blue-600">Ask for Developer</button>} /><div className="mt-6 grid gap-4 md:grid-cols-3">{[['1', 'Open the guide', 'Use our ready-made prompt to generate a valid OpenAPI 3.0.3 YAML file.'], ['2', 'Add real API details', 'Only include endpoints, fields, and login details that actually exist in your store.'], ['3', 'Save the file', 'Download the result as .yaml, .yml, or .json, then upload it in the next step.']].map(([number, title, copy]) => <div key={number} className="rounded-xl border border-slate-200 bg-slate-50 p-5"><span className="text-2xl font-extrabold text-blue-600">{number}</span><h2 className="mt-3 font-bold text-slate-900">{title}</h2><p className="mt-1 text-sm leading-6 text-slate-500">{copy}</p></div>)}</div><a href="https://github.com/youssefbassem42/AiCommerce-AI-Service/blob/main/docs/ecommerce-schema-generation-prompt.md" target="_blank" rel="noreferrer" className="mt-6 inline-flex min-h-11 items-center gap-2 rounded-lg border border-blue-600 px-4 py-2.5 text-sm font-semibold text-blue-600 hover:bg-blue-50"><Copy className="h-4 w-4" />Open and copy the schema prompt</a><Button className="mt-5" onClick={onContinue}>I have my schema file <ArrowRight className="h-4 w-4" /></Button></div>; }
function SchemaUploadStep({ complete, loading, onBack, onUpload, onContinue }) { return <div><Back onClick={onBack} /><StepHeading icon={FileJson} eyebrow="Integration · 2 of 4" title="Upload OpenAPI schema" copy="Upload the YAML or JSON file you generated. We will analyze the endpoints, create the connection, and start the first sync." /><div className="mt-6 rounded-2xl border border-dashed border-blue-200 bg-blue-50/50 p-8 text-center"><FileJson className="mx-auto h-10 w-10 text-blue-600" /><h2 className="mt-4 font-bold text-slate-900">OpenAPI 3.0 file</h2><p className="mt-2 text-sm text-slate-500">Accepted formats: .yaml, .yml, and .json</p><div className="mt-5 flex justify-center"><FilePicker accept=".json,.yaml,.yml,application/json,application/yaml,text/yaml" loading={loading} label={complete ? 'Upload another schema' : 'Choose schema file'} onChange={onUpload} /></div>{complete && <p className="mt-4 inline-flex items-center gap-2 text-sm font-semibold text-emerald-700"><Check className="h-4 w-4" />Schema analyzed and connection started.</p>}</div><Button className="mt-6" disabled={!complete} onClick={onContinue}>Continue to policies <ArrowRight className="h-4 w-4" /></Button></div>; }
...
function WidgetSetupStep({ onBack, onFinish }) { return <div><Back onClick={onBack} /><StepHeading icon={Code2} eyebrow="Integration · 4 of 4" title="Install your storefront widget" copy="Create your live widget key here, then copy the installation code into your e-commerce site. You can manage or disable the key later from Profile." /><div className="mt-6"><WidgetAccessPanel /></div><Button className="mt-6" onClick={onFinish}>Open Merchant Dashboard <ArrowRight className="h-4 w-4" /></Button></div>; }
