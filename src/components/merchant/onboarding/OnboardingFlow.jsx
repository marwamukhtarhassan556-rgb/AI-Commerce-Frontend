...
const initialStore = { name: '', description: '', platform: 'custom', shopDomain: '', adminEmail: '', adminPassword: '', currency: 'USD', language: 'en', timezone: 'UTC' };
...
function StoreStep({ value, onChange, onSubmit, loading, onBack }) { return <form onSubmit={onSubmit}><Back onClick={onBack} /><h1 className="text-2xl font-bold">Create your store</h1><p className="mt-2 mb-6 text-sm text-slate-500">Add your store details and the administrator credentials used to connect it.</p><Fields fields={[['name', 'Store Name', 'text'], ['description', 'Description', 'text'], ['shopDomain', 'Website Domain', 'url'], ['adminEmail', 'Admin email', 'email'], ['adminPassword', 'Admin password', 'password'], ['currency', 'Currency', 'text'], ['language', 'Language', 'text'], ['timezone', 'Timezone', 'text']]} value={value} onChange={onChange} /><p className="mt-3 text-xs text-slate-500">Your administrator password is sent securely to connect this store and is never saved in the browser.</p><Button className="mt-6" loading={loading} type="submit">Create Store</Button></form>; }
