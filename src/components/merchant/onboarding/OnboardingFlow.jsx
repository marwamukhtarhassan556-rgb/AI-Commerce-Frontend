...
export default function OnboardingFlow() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (active) setTrialStatus(normalizeSubscription(data));
      } catch {
        // Temporary compatibility while the current-subscription endpoint is being added.
        try { const { data } = await subscriptionsApi.getTrialStatus(); if (active) setTrialStatus(normalizeSubscription(data)); } catch { /* plans remain available */ }
      }
    };
    void load();
    return () => { active = false; };
  ...
  return <main className="onboarding-shell min-h-screen text-slate-800">
    ...
    <section className={`onboarding-card mx-auto w-full px-8 py-12 sm:px-14 sm:py-16 ${step === 3 ? 'onboarding-plan-page onboarding-plans-card' : ''}`} style={{ maxWidth: step === 3 ? '1200px' : '1000px' }}>
      ...
      {step === 3 && <PlansStep plans={plans} trialStatus={trialStatus} loading={plansLoading || loading} onSelect={openPlan} onCreateStore={() => setStep(4)} onBack={back} />}
