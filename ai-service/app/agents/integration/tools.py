...
from app.agents.integration.schemas import IntegrationMappingReport, FeatureAnalysis
...
async def analyze_spec_with_llm(
    ...
    request = ChatRequest(
        messages=[
            MessageDTO(role="system", content="You are an e-commerce API integration expert. Return ONLY valid JSON matching the requested schema. No markdown, no explanation."),
            MessageDTO(role="user", content=analyze_prompt),
        ],
        model=model,
        json_mode=True,
        temperature=0.1,
