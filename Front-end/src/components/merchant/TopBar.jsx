...
export default function TopBar() {
  ...
  return <header className="merchant-topbar fixed top-0 right-0 left-0 z-40 flex min-h-16 items-center gap-3 border-b border-slate-200 bg-white px-3 py-2 lg:left-[280px] sm:px-5">
    ...
    <div className="ml-auto flex shrink-0 items-center gap-1 text-slate-400"><button className="hidden rounded-full p-2 hover:bg-slate-50 sm:block"><Bell className="h-5 w-5" /></button><button className="hidden rounded-full p-2 hover:bg-slate-50 md:block"><Settings className="h-5 w-5" /></button><button className="hidden rounded-full p-2 hover:bg-slate-50 md:block"><HelpCircle className="h-5 w-5" /></button><div className="hidden border-l border-slate-100 pl-3 text-right lg:block"><p className="text-sm font-bold text-slate-900">Marwa Mukhtar</p><p className="text-[10px] text-slate-400">MANAGE ACCOUNT</p></div><div className="flex h-9 w-9 items-center justify-center rounded-full bg-indigo-600 text-sm font-bold text-white">MM</div><LogoutButton variant="ghost" className="px-2 py-2 text-xs" /></div>
  </header>;
}
