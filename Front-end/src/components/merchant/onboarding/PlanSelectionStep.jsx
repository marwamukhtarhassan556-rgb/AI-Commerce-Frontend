...
export default function PlanSelectionStep({ onNext, onBack }) {
  ...
  const handleSubscribe = async (plan) => {
    ...
    setSubscribing(true);
    try {
      const { data } = await api.post('/api/seller/subscriptions/subscribe', { planId: plan.id });
      const status = String(data?.status || data?.subscriptionStatus || '').toLowerCase();
      if (status !== 'active') throw new Error('Subscription was not activated.');
      setActivePlan({ name: plan.name, price: plan.price });
