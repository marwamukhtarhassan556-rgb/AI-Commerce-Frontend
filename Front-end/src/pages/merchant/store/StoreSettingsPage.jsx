...
export default function StoreSettingsPage() {
  ...
  useEffect(() => {
    ...
    storesApi.getById(storeId)
      .then(({ data }) => mounted && setStore({ ...emptyStore, ...data }))
  ...
  const updatePromoCapability = async (event) => {
    ...
    if (!storeId) return;
    setHasPromoCode(nextValue);
    setUpdatingPromoCapability(true); setMessage('');
    try {
      await storeCapabilitiesApi.update({ storeId, hasPromoCode: nextValue });
        ...
        {loading ? <div className="rounded-xl bg-white p-8 text-center text-sm text-on-surface-variant">Loading store…</div> : <>
          ...
          <section className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
            ...
            <div className="store-capability-row">
              ...
              <label className={`store-capability-toggle ${updatingPromoCapability ? 'is-saving' : ''}`}><input type="checkbox" checked={hasPromoCode} onChange={updatePromoCapability} disabled={updatingPromoCapability || !storeId} /><span className="store-capability-switch" aria-hidden="true" /><span className="text-sm font-semibold" aria-live="polite">{!storeId ? 'Select a store first' : updatingPromoCapability ? 'Saving…' : hasPromoCode ? 'Promo codes on' : 'Promo codes off'}</span></label>
            </div>
          </section>
