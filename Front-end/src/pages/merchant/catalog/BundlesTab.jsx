...
export default function BundlesTab({ storeId }) {
  ...
  const copyAndTrackBundle = async (bundle) => {
    const promoCode = bundle.promo_code;
    const productIds = (bundle.products || []).map((product) => product.product_id).filter(Boolean);
    if (!promoCode || productIds.length === 0) {
      setError('This suggestion is missing the promo code or product IDs required for tracking.');
      return;
    }
    try {
      await navigator.clipboard.writeText(promoCode);
      const totalOriginal = Number(bundle.total_original) || 0;
      const totalDiscount = Number(bundle.total_discount) || 0;
      await bundlesApi.track({ store_id: storeId, promo_code: promoCode, product_ids: productIds, discount_pct: Number(bundle.discount_pct) || (totalOriginal ? (totalDiscount / totalOriginal) * 100 : 0), total_discount: totalDiscount, total_original: totalOriginal });
      setCopiedPromo(promoCode);
      loadBundles();
    } catch (error) { setError(requestError(error, 'Promo code could not be copied and tracked.')); }
  };
  ...
  return <div>
    ...
    {showSuggestion && <section className="border-b border-outline-variant/30 bg-surface-container-low p-5">
      <div className="mb-4"><h3 className="flex items-center gap-2 text-sm font-bold"><Sparkles className="h-4 w-4 text-primary" />AI Bundle Suggestion</h3><p className="mt-1 text-xs text-on-surface-variant">Describe the customer need and budget. The AI will suggest a bundle from this store.</p></div>
      <form onSubmit={getSuggestion} className="grid gap-3 md:grid-cols-[1fr_12rem_auto]"><input required value={suggestionInput.message} onChange={(event) => setSuggestionInput((value) => ({ ...value, message: event.target.value }))} placeholder="e.g. A laptop bundle under $1500 for programming" className="rounded-xl border border-outline-variant bg-white px-3 py-2.5 text-sm" /><input value={suggestionInput.customerId} onChange={(event) => setSuggestionInput((value) => ({ ...value, customerId: event.target.value }))} placeholder="Customer ID (optional)" className="rounded-xl border border-outline-variant bg-white px-3 py-2.5 text-sm" /><button disabled={suggesting} className="rounded-xl bg-primary px-4 py-2.5 text-sm font-bold text-white disabled:opacity-50">{suggesting ? 'Thinking…' : 'Suggest'}</button></form>
      {suggestion && <div className="mt-4 grid gap-3 lg:grid-cols-2">{(suggestion.bundles || []).map((bundle, index) => <article key={index} className="rounded-xl border border-primary/20 bg-white p-4"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-bold text-primary">Suggested bundle #{index + 1}</p><ul className="mt-2 space-y-1">{(bundle.products || []).map((product) => <li key={product.product_id} className="text-sm font-medium text-on-surface">{product.product_title} <span className="text-on-surface-variant">— {product.price_after_discount} after {product.discount_pct}% off</span></li>)}</ul></div>{bundle.promo_code && <button onClick={() => copyAndTrackBundle(bundle)} className="inline-flex items-center gap-1.5 rounded-full bg-emerald-50 px-2.5 py-1 text-xs font-bold text-emerald-700 hover:bg-emerald-100" title="Copy promo code and record this copy"><Copy className="h-3.5 w-3.5" />{copiedPromo === bundle.promo_code ? 'Copied' : bundle.promo_code}</button>}</div><div className="mt-3 flex flex-wrap gap-x-4 gap-y-1 border-t border-outline-variant/20 pt-3 text-xs"><span>Original: <b>{bundle.total_original}</b></span><span className="text-emerald-600">Discount: <b>{bundle.total_discount}</b></span><span>Final: <b>{bundle.total_after_discount}</b></span></div></article>)}</div>}
      {suggestion?.rationale && <p className="mt-3 text-xs text-on-surface-variant">{suggestion.rationale}</p>}
    </section>}
